...
from chronus.datasets.preprocess import tokenizer_image_token, tokenizer_speech_image_token, tokenizer_speech_question_image_token, tokenizer_speech_token
...
parser.add_argument("--results_path", type=str, required=True, help="Path to save the output results JSON file")
...
for sample in tqdm(samples):
    ...
    if modality == "video":
        ...
        if total_frame_num>args.frame_num:
            frame_idx = np.linspace(0, total_frame_num - 1, args.frame_num, dtype=int).tolist() 
        else: 
        ...
        time_interval= video_duration/(min(total_frame_num,args.frame_num)-1)
        timestamp=[]
    ...
    if audio_path and image_path: # image + speech instruction
        qs = DEFAULT_IMAGE_TOKEN + "\n" + "User's question in speech: " + DEFAULT_SPEECH_TOKEN + '\n'
    elif audio_path and video_path: # video + audio
        qs = DEFAULT_SPEECH_TOKEN + DEFAULT_IMAGE_TOKEN + "\n" + qs
    elif audio_path: # audio + text
    ...
    results.append({"id": sample["id"], "question": qs, "output": outputs})
