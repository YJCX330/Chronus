...
from chronus.constants import IGNORE_INDEX, DEFAULT_SPEECH_TOKEN, SPEECH_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, IMAGE_TOKEN_INDEX
from chronus.train.chronus_trainer import ChronusTrainer

from chronus import conversation as conversation_lib
from chronus.model import *
from chronus.datasets.preprocess import tokenizer_speech_token
...
from chronus.mm_utils import process_anyres_video, process_anyres_highres_image
...
def read_video_file(file_path,cur_frames_upbound):
    from decord import VideoReader, cpu
    vr = VideoReader(file_path, ctx=cpu(0))
    total_frame_num = len(vr)
    if cur_frames_upbound>0 and total_frame_num>cur_frames_upbound:
        frame_idx = np.linspace(0, total_frame_num - 1, cur_frames_upbound, dtype=int).tolist() 
    else: 
    ...
    time_interval= video_duration/(min(total_frame_num,cur_frames_upbound)-1)
    timestamp=[]
    for i in range(len(frame_idx)):
        time_number=str("{:.1f}".format(time_interval*i))
        timestamp_text='second{'+time_number+'}'
        timestamp.append(timestamp_text)
    return video,frame_idx,timestamp,time_interval
...
class LazySupervisedDataset(Dataset):
    ...
    def process_video(self, video_file):
        ...
        if isinstance(video_file, str):
            video,frame_idx, timestamp, time_interval = read_video_file(video_file,cur_frames_upbound)
        else:
        ...
        return (video_processed, (384, 384), "video"), frame_idx,  timestamp, time_interval
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'video' in sources[0] and 'audio' in sources[0]:  # video + audio
            ...
            # audio = [audio]
            video, _ , timestamp, time_interval = self.process_video(video_file)
            video = [video]
        ...
        if timestamp:
            timestamp1=[]
            for t_item in timestamp:
                timestamp1.append(torch.tensor(self.tokenizer(t_item)['input_ids']))
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        timestamp=[instance['timestamp'] for instance in instances]
        time_interval=[instance['time_interval'] for instance in instances]
        batch['timestamp']=timestamp
        batch['time_interval']=time_interval
...
def train():
    ...
    model = ChronusQwenForCausalLM.from_pretrained(
    ...
    trainer = ChronusTrainer(model=model,
                    tokenizer=tokenizer,
                    args=training_args,
                    **data_module)
