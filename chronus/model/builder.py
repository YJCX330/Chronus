...
from chronus.model.speech_encoder.builder import build_speech_encoder
...
def load_pretrained_model(model_path, model_base, is_lora=False, s2s=False, load_8bit=False, load_4bit=False, device="cuda", use_flash_attn=False, **kwargs):
    ...
    model_cls = ChronusQwenForCausalLM

    # Load Chronus model
    if is_lora:
        assert model_base is not None, "model_base is required for LoRA models."
        from chronus.model.language_model.chronus_qwen import ChronusConfigQwen
        lora_cfg_pretrained = ChronusConfigQwen.from_pretrained(model_path)
        tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
        print('Loading Chronus from base model...')
        model = model_cls.from_pretrained(model_base, low_cpu_mem_usage=False, config=lora_cfg_pretrained, **kwargs)
        print('Loading additional Chronus weights...')
