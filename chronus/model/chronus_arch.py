...
from chronus.constants import IGNORE_INDEX, SPEECH_TOKEN_INDEX
from chronus.utils import lengths_to_padding_mask

from .multimodal_encoder.builder import build_vision_tower
from .multimodal_resampler.builder import build_vision_resampler
from .multimodal_projector.builder import build_vision_projector

from chronus.constants import IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN

class ChronusMetaModel:

    def __init__(self, config):
        super(ChronusMetaModel, self).__init__(config)
...
class ChronusMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_speech_vision_text(
        ...
        speech, speech_lengths, speech_chunks, speech_wav, images, modalities, image_sizes=None, images_highres=None, timestamp=None, time_interval=0
    ):
        speech_encoder = self.get_speech_encoder()
        ...
        speech_begin=self.get_speech_projector().speech_begin.reshape(1, -1)
        speech_end=self.get_speech_projector().speech_end.reshape(1, -1)
        image_begin=self.get_vision_projector().image_begin.reshape(1, -1)
        image_end=self.get_vision_projector().image_end.reshape(1, -1)
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            cur_timestamp=timestamp[batch_idx]
            cur_time_interval=time_interval[batch_idx]
            if cur_timestamp: 
                cur_timestamp_embeds=[self.get_model().embed_tokens(x) for x in cur_timestamp]
                slice_num=len(cur_timestamp)
                cur_images_features = image_features[cur_image_idx]
                cur_image_idx += 1
                cur_images_features_split1= cur_images_features.view(slice_num, -1, 3584)
                cur_images_features_split=[]
                for image in cur_images_features_split1:
                    cur_images_features_split.append(torch.cat([image_begin.to(image.dtype), image, image_end.to(image.dtype)], dim=0))
                if num_speech_images>num_images:
                    cur_speech_features = speech_features[cur_speech_idx]
                    cur_speech_idx += 1
                    audio_token_num= [round(5*cur_time_interval*i) for i in range(slice_num)]
                    audio_token_num[0]=-1
                    audio_token_num[-1]=len(cur_speech_features)-1
                    cur_speech_features_split=[]
                    for i in range(len(audio_token_num)-1):
                        cur_speech_features_split.append(torch.cat([speech_begin.to(cur_speech_features.dtype), cur_speech_features[audio_token_num[i]+1:audio_token_num[i+1]+1], speech_end.to(cur_speech_features.dtype)], dim=0))
                for i in range(num_speech_images + 1):
                    ...
                    if i < 1: 
                        for j in range(slice_num):
                            cur_new_input_embeds.append(cur_timestamp_embeds[j])
                            cur_new_labels.append(torch.full((cur_timestamp_embeds[j].shape[0],), IGNORE_INDEX, device=cur_labels.device, dtype=cur_labels.dtype))
                            cur_new_input_embeds.append(cur_images_features_split[j])
                            cur_new_labels.append(torch.full((cur_images_features_split[j].shape[0],), IGNORE_INDEX, device=cur_labels.device, dtype=cur_labels.dtype))
                            if num_speech_images>num_images and j <slice_num-1:
                                cur_new_input_embeds.append(cur_speech_features_split[j])
                                cur_new_labels.append(torch.full((cur_speech_features_split[j].shape[0],), IGNORE_INDEX, device=cur_labels.device, dtype=cur_labels.dtype))
            else:
                for i in range(num_speech_images + 1):
                    cur_new_input_embeds.append(cur_input_embeds_no_speech_image[i])
                    cur_new_labels.append(cur_labels_nospeech_image[i])
                    if i < num_speech_images:
                        if i < num_images:
                            cur_images_features = torch.cat([image_begin.to(image_features.dtype), image_features[cur_image_idx], image_end.to(image_features.dtype)], dim=0)
                            cur_image_idx += 1
                            ...
                            cur_speech_features = torch.cat([speech_begin.to(speech_features[cur_speech_idx].dtype), speech_features[cur_speech_idx], speech_end.to(speech_features[cur_speech_idx].dtype)], dim=0)
                            cur_speech_idx += 1
